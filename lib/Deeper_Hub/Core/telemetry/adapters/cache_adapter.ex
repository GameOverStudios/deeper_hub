defmodule DeeperHub.Core.Telemetry.Adapters.CacheAdapter do
  ...
  defp handle_stats_event(_event, measurements, metadata, config) do
    ...
    if metadata[:name] == cache_name do
      # Publica métricas do cache para telemetria
      :telemetry.execute(
        [String.to_atom(prefix), :periodic_metrics],
        %{
          size: measurements[:size] || 0,
          memory: measurements[:memory] || 0,
          hit_rate: measurements[:hit_rate] || 0,
          hits: measurements[:hits] || 0,
          misses: measurements[:misses] || 0,
          expired_count: measurements[:expired] || 0
        },
        %{cache_name: cache_name}
      )
    end
  ...
  defp handle_operation_event(_event, measurements, metadata, config) do
    ...
    if metadata[:name] == cache_name do
      ...
      :telemetry.execute(
        [String.to_atom(prefix), :operation],
        %{
          count: 1,
