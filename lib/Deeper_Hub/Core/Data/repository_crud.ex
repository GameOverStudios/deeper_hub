defmodule Deeper_Hub.Core.Data.RepositoryCrud do
  ...
  def transaction(fun, opts \\ []) do
    ...
    # Usar diretamente a função transaction do Repo
    # Isso garante que a conexão seja gerenciada corretamente
    result = Repo.transaction(fun, opts)
    ...
    case result do
      {:ok, value} ->
        # Emite evento de telemetria para transação bem-sucedida
      ...
      {:error, reason} ->
        # Emite evento de telemetria para transação com erro
        TelemetryEvents.execute_db_transaction(
          %{duration: duration, count: 1},
  ...
  def insert(schema, attrs) do
    ...
    case result do
      {:ok, struct} ->
        ...
        CacheManager.put_in_cache(schema, id, struct)
  ...
  def get(schema, id) do
    ...
    result = case CacheManager.get_from_cache(schema, id) do
      {:ok, value} ->
      ...
      :not_found ->
        ...
        DBConn.run(nil, fn ->
          case Repo.get(schema, id) do
            ...
            record ->
              # Armazena no cache para futuras consultas
              CacheManager.put_in_cache(schema, id, record)
  ...
  def update(struct, attrs) do
    ...
    case result do
      {:ok, updated_struct} ->
        ...
        # Atualiza o cache
        CacheManager.put_in_cache(struct.__struct__, id, updated_struct)
  ...
  def delete(struct) do
    ...
    case result do
      {:ok, deleted_id} ->
        ...
        CacheManager.invalidate_cache(struct.__struct__, deleted_id)
  ...
  def list(schema, opts \\ []) do
    ...
    result = DBConn.run(nil, fn ->
      ...
      query = Repository.apply_limit_offset(query, opts)
