defmodule DeeperHub.Core.Cache.Resilience.FallbackHandler do
  ...
  def with_fallback(cache_name, key, fallback_fn, opts \\ []) do
    ...
    case Cachex.get(cache_name, key) do
      ...
      {:error, reason} ->
        ...
        DeeperHub.Core.Logger.warn("Falha no cache #{inspect(cache_name)}, usando fallback (#{inspect(strategy)}): #{inspect(reason)}")
  ...
  def invalidate(cache_name, key, _opts \\ []) do
    ...
    case Cachex.del(cache_name, key) do
      ...
      {:error, reason} ->
        DeeperHub.Core.Logger.warn("Erro ao invalidar cache para #{inspect(key)}: #{inspect(reason)}")
        {:error, reason}
    end
  end
