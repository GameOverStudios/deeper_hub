# Migração Elixir: Criar Tabela `bx_persons_meta_locations`

Este módulo de migração Elixir é responsável por criar a tabela `bx_persons_meta_locations` no banco de dados SQLite. Esta tabela armazena informações de localização estruturadas (latitude, longitude, país, cidade, etc.) para perfis de pessoas.

**Dependências:** `sys_profiles`

## Código da Migração (`lib/deeper/content/persons/migrations/create_bx_persons_meta_locations_table.ex`)

```elixir
defmodule Deeper.Content.Persons.Migrations.CreateBxPersonsMetaLocationsTable do
  @moduledoc \"\"\"
...
*   `object_id`: `INT UNSIGNED` com `PRIMARY KEY` (MySQL) -> `INTEGER PRIMARY KEY NOT NULL` (SQLite).
*   `lat`, `lng`: `DOUBLE` (MySQL) -> `REAL` (SQLite).
*   `country`, `state`, `city`, `zip`, `street`, `street_number`: `VARCHAR` (MySQL) -> `TEXT` (SQLite).
*   **Índices:** O índice composto `country_state_city` foi adaptado. Um índice em `(lat, lng)` é adicionado, o que é útil para buscas geoespaciais (embora o SQLite não tenha funções espaciais tão avançadas quanto o PostGIS, por exemplo, um índice aqui ainda ajuda).
