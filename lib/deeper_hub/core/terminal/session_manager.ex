defmodule DeeperHub.Core.Terminal.SessionManager do
  ...
  alias DeeperHub.Core.Terminal.Session
  ...
  def handle_call({:execute_command, session_id, command, client_pid}, from, state) do
    ...
    case Map.get(state.sessions, session_id) do
      nil ->
        Logger.warn("SessionManager: Tentativa de executar comando em sessão não encontrada: #{session_id}")
        {:reply, {:error, :session_not_found}, state}

      %Session{port: nil} = session -> # Caso o port tenha sido explicitamente setado para nil
      ...
      %Session{port: port_obj} = session when is_port(port_obj) -> # Renomeado port para port_obj
        ...
        if Port.info(port_obj) != nil do
          ...
          Logger.warn("SessionManager: Tentativa de executar comando na sessão #{session_id}, mas o port #{inspect(port_obj)} não está mais ativo.")
      ...
      %Session{port: invalid_port_val} = _session -> # Port não é um port válido
  ...
  def handle_call({:terminate_session, session_id}, _from, state) do
    case Map.get(state.sessions, session_id) do
      nil ->
        {:reply, {:error, :session_not_found}, state}
      %Session{port: port, safety_timer_ref: timer_ref} ->
  ...
  def handle_call(:terminate_all_sessions, _from, state) do
    ...
    Enum.each(state.sessions, fn {_id, %Session{port: port, safety_timer_ref: timer_ref}} ->
      if timer_ref, do: Process.cancel_timer(timer_ref)
  ...
  def handle_info({:command_safety_timeout, _original_caller_from, session_id, client_pid_timed_out}, state) do
    case Map.get(state.sessions, session_id) do
      nil ->
        Logger.warn("Timeout de segurança para sessão #{session_id} que não existe mais.")
        {:noreply, state}
      %Session{safety_timer_ref: nil} ->
        Logger.debug("Timeout de segurança para sessão #{session_id} ignorado, comando/timer já tratado.")
        {:noreply, state}
      %Session{} = session ->
        Logger.warn("Timeout de segurança ACIONADO para sessão #{session_id}.")
  ...
  def handle_info({port_obj, {:exit_status, status}}, state) do # Renomeado port para port_obj
    case find_session_by_port(port_obj, state.sessions) do
      {session_id, %Session{safety_timer_ref: timer_ref, client_pid: client_pid}} ->
        Logger.warn("Porta da sessão #{session_id} (port: #{inspect port_obj}) terminou com status: #{status}.")
        if timer_ref, do: Process.cancel_timer(timer_ref)
      ...
      nil ->
        ...
        Logger.warn("Porta #{inspect(port_obj)} terminou, mas nenhuma sessão correspondente encontrada no estado atual.")
        {:noreply, state}
    end
  end
  ...
  def handle_info({port_obj, {:data, data_content}}, state) do # Renomeado port para port_obj, data para data_content
     Logger.warn("SessionManager: Recebida data órfã do port #{inspect port_obj}: #{inspect data_content}")
  ...
  def terminate(_reason, state) do
    Logger.info("SessionManager terminando. Motivo: #{inspect _reason}. Encerrando todas as sessões.")
    Enum.each(state.sessions, fn {_id, %Session{port: port, safety_timer_ref: timer_ref}} ->
      if timer_ref, do: Process.cancel_timer(timer_ref)
      safe_close_port(port)
    end)
  ...
  defp find_session_by_port(port_to_find, sessions) do # Renomeado port para port_to_find
    Enum.find(sessions, fn {_id, %Session{port: p}} -> p == port_to_find end)
  end
end
