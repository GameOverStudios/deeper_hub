    ...
    try do
      ...
      adapters = if :cache in enabled_adapters do
        case CacheAdapter.setup(opts) do
          ...
          {:error, reason} ->
            DeeperHub.Core.Logger.warning("Falha ao inicializar adaptador de telemetria do cache: #{inspect(reason)}")
            adapters
        end
      else
        adapters
      ...
      adapters = if :http in enabled_adapters do
        case HttpAdapter.setup(opts) do
          ...
          {:error, reason} ->
            DeeperHub.Core.Logger.warning("Falha ao inicializar adaptador de telemetria HTTP: #{inspect(reason)}")
            adapters
        end
      else
        adapters
      ...
      adapters = if :network in enabled_adapters do
        case NetworkAdapter.setup(opts) do
          ...
          {:error, reason} ->
            DeeperHub.Core.Logger.warning("Falha ao inicializar adaptador de telemetria de rede: #{inspect(reason)}")
            adapters
        end
      else
        adapters
      ...
      adapters = if :security in enabled_adapters do
        case SecurityAdapter.setup(opts) do
          ...
          {:error, reason} ->
            DeeperHub.Core.Logger.warning("Falha ao inicializar adaptador de telemetria de segurança: #{inspect(reason)}")
            adapters
        end
      else
        adapters
      ...
      adapters = if :database in enabled_adapters do
        case DatabaseAdapter.setup(opts) do
          ...
          {:error, reason} ->
            DeeperHub.Core.Logger.warning("Falha ao inicializar adaptador de telemetria de banco de dados: #{inspect(reason)}")
            adapters
        end
      else
        adapters
