defmodule DeeperHub.Core.Data.Migrations.BxGlossaryTerms do
  ...
  def up do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_glossary_terms: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_glossary_terms: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
