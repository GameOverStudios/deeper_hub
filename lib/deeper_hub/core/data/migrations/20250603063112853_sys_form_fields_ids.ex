defmodule DeeperHub.Core.Data.Migrations.SysFormFieldsIds do
  ...
  def up do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de sys_form_fields_ids: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de sys_form_fields_ids: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
