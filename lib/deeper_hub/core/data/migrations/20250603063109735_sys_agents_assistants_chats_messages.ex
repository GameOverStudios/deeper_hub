defmodule DeeperHub.Core.Data.Migrations.SysAgentsAssistantsChatsMessages do
  ...
  def up do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de sys_agents_assistants_chats_messages: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de sys_agents_assistants_chats_messages: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
