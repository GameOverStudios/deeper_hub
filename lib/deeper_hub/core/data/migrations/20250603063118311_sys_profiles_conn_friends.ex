defmodule DeeperHub.Core.Data.Migrations.SysProfilesConnFriends do
  ...
  def up do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de sys_profiles_conn_friends: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de sys_profiles_conn_friends: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
