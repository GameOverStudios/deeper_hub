...
defmodule DeeperHub.Core.Data.Migrations.CreateBxPersonsFavoritesTrackTable do
  ...
  def up do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela bx_persons_favorites_track: #{inspect(reason)}",
          module: __MODULE__
        )
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela bx_persons_favorites_track: #{inspect(reason)}",
          module: __MODULE__
        )
