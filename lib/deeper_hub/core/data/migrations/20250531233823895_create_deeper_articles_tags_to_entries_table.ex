# Migração gerada com ID único: V1748745503895 em 2025-05-31 23:38:23
defmodule DeeperHub.Core.Data.Migrations.CreateDeeperArticlesTagsToEntriesTable do
  # Migração gerada com ID único: V1748745503895 em 2025-05-31 23:38:23
  ...
  def up do
    ...
    |> tap(fn
      {:ok, _} ->
        Logger.info("Tabela deeper_articles_tags_to_entries criada com sucesso.",
          module: __MODULE__
        )

      {:error, reason} ->
        Logger.error("Falha ao criar tabela deeper_articles_tags_to_entries: #{inspect(reason)}",
          module: __MODULE__
        )
    end)
  end
  ...
  def down do
    ...
    |> tap(fn
      {:ok, _} ->
        Logger.info("Tabela deeper_articles_tags_to_entries removida com sucesso.",
          module: __MODULE__
        )

      {:error, reason} ->
        Logger.error(
          "Falha ao remover tabela deeper_articles_tags_to_entries: #{inspect(reason)}",
          module: __MODULE__
        )
    end)
  end
