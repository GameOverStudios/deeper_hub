defmodule DeeperHub.Core.Data.Migrations do
    ...
    cond do
      ...
      true ->
        if attempt < max_attempts do
          Logger.warning("Pool de conexões #{inspect(pool_name)} não encontrado ou não está ativo. Aguardando #{wait_time_ms}ms antes da próxima tentativa...", module: __MODULE__)
  ...
  defp try_test_query(pool_name, attempt, max_attempts, wait_time_ms) do
    ...
    try do
      ...
      if Process.alive?(Process.whereis(pool_name)) do
        ...
        case DeeperHub.Core.Data.Repo.query("SELECT 1 AS test;") do
          {:ok, _rows} ->
            ...
            :ok
          {:error, error} ->
            Logger.warning("Erro ao executar consulta de teste via Repo: #{inspect(error)}", module: __MODULE__)
      ...
      error ->
        Logger.warning("Exceção ao verificar pool de conexões: #{inspect(error)}", module: __MODULE__)
        retry_or_fail(attempt, max_attempts, wait_time_ms)
    end
  end
  ...
  defp retry_or_fail(attempt, max_attempts, wait_time_ms) do
    if attempt < max_attempts do
      Logger.warning("Tentando novamente em #{wait_time_ms}ms (tentativa #{attempt}/#{max_attempts})...", module: __MODULE__)
      Process.sleep(wait_time_ms)
      wait_for_pool(attempt + 1, max_attempts, wait_time_ms)
    else
