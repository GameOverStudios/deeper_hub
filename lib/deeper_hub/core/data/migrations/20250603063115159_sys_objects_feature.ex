defmodule DeeperHub.Core.Data.Migrations.SysObjectsFeature do
  ...
  def up do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de sys_objects_feature: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de sys_objects_feature: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
