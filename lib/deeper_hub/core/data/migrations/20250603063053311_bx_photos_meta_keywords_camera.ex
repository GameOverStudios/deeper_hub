defmodule DeeperHub.Core.Data.Migrations.BxPhotosMetaKeywordsCamera do
  ...
  def up do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_photos_meta_keywords_camera: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_photos_meta_keywords_camera: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
