defmodule DeeperHub.Core.Data.Migrations.BxTimelineReportsTrack do
  ...
  def up do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_timeline_reports_track: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_timeline_reports_track: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
