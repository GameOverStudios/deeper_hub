# Migração Elixir: Criar Tabela `bx_persons_scores` (Agregação de Scores para Pessoas)

Este módulo de migração Elixir cria a tabela `bx_persons_scores` no SQLite, para agregação de pontuações (up/down votes) de perfis de pessoas.

Esta é um exemplo de uma tabela `table_main` referenciada em `sys_objects_score`.

## Código da Migração (`lib/deeper/core/data/migrations/create_bx_persons_scores_table.ex`)

```elixir
defmodule Deeper.Core.Data.Migrations.CreateBxPersonsScoresTable do
  @moduledoc \"\"\"
...
*   `object_id`: Corresponde ao `id` da tabela `bx_persons_data` (o perfil que está sendo pontuado).
*   `count_up`: Número total de votos \"up\".
*   `count_down`: Número total de votos \"down\".
*   A pontuação líquida (`score`) é calculada como `count_up - count_down`.