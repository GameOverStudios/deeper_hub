...
Este módulo de migração Elixir é responsável por criar a tabela `sys_objects_cmts` no banco de dados SQLite. Esta tabela é fundamental para o sistema de comentários genérico do UNA, pois define cada \"objeto de comentários\" (ou seja, cada instância de um sistema de comentários para um tipo de conteúdo específico), incluindo qual tabela SQL armazena os comentários e várias configurações de comportamento.

## Código da Migração (`lib/deeper/interaction_systems/comments/migrations/create_sys_objects_cmts_table.ex`)

```elixir
defmodule Deeper.InteractionSystems.Comments.Migrations.CreateSysObjectsCmtsTable do
  @moduledoc \"\"\"
...
*   Colunas `INT` e `TINYINT` (para booleanos) do MySQL -> `INTEGER` no SQLite.
*   `BrowseType` e `PostFormPosition`: Eram `VARCHAR` no dump original, mas funcionam como ENUMs. Foram adicionadas `CHECK` constraints para clareza, baseadas nos valores comuns do UNA.
*   A coluna `\"Table\"` está entre aspas para evitar conflito com a palavra reservada SQL.