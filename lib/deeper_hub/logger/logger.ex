...
defmodule DeeperHub.Logger do
  ...
  DeeperHub.Logger.info("Iniciando operação", user_id: "123")
  DeeperHub.Logger.error("Falha na operação", error: err, operation: :process_data)
  ```
  """
  ...
  ## Exemplos
      ...
      iex> require DeeperHub.Logger
      iex> DeeperHub.Logger.debug("Mensagem de debug")
      :ok

      iex> require DeeperHub.Logger
      iex> DeeperHub.Logger.debug("Detalhes da operação", user_id: "123", operation: :query)
      :ok
  """
  ...
  defmacro debug(message, metadata \\ []) do
    ...
    quote do
      DeeperHub.Logger.__log__(:debug, unquote(message), Keyword.put(unquote(metadata), :module, unquote(caller_module_atom)))
    end
  end
  ...
  ## Exemplos
      ...
      iex> require DeeperHub.Logger
      iex> DeeperHub.Logger.info("Mensagem informativa")
      :ok

      iex> require DeeperHub.Logger
      iex> DeeperHub.Logger.info("Usuário autenticado", user_id: "123", ip: "192.168.1.1")
      :ok
  """
  ...
  defmacro info(message, metadata \\ []) do
    caller_module_atom = __CALLER__.module
    quote do
      DeeperHub.Logger.__log__(:info, unquote(message), Keyword.put(unquote(metadata), :module, unquote(caller_module_atom)))
    end
  end
  ...
  ## Exemplos
      ...
      iex> require DeeperHub.Logger
      iex> DeeperHub.Logger.warning("Alerta importante")
      :ok

      iex> require DeeperHub.Logger
      iex> DeeperHub.Logger.warning("Tentativa suspeita de login", user_id: "123", ip: "203.0.113.1")
      :ok
  """
  ...
  defmacro warning(message, metadata \\ []) do
    caller_module_atom = __CALLER__.module
    quote do
      DeeperHub.Logger.__log__(:warn, unquote(message), Keyword.put(unquote(metadata), :module, unquote(caller_module_atom)))
    end
  end
  ...
  ## Exemplos
      ...
      iex> require DeeperHub.Logger
      iex> DeeperHub.Logger.error("Ocorreu um erro grave")
      :ok

      iex> require DeeperHub.Logger
      iex> DeeperHub.Logger.error("Falha na conexão com o banco de dados", error: err, operation: :save_user)
      :ok
  """
  ...
  defmacro error(message, metadata \\ []) do
    caller_module_atom = __CALLER__.module
    quote do
      DeeperHub.Logger.__log__(:error, unquote(message), Keyword.put(unquote(metadata), :module, unquote(caller_module_atom)))
    end
  end
  ...
  ## Exemplos
      ...
      iex> require DeeperHub.Logger
      iex> DeeperHub.Logger.critical("Falha crítica no sistema!")
      :ok

      iex> require DeeperHub.Logger
      iex> DeeperHub.Logger.critical("Servidor indisponível", service: :authentication, impact: :high)
      :ok
  """
  ...
  defmacro critical(message, metadata \\ []) do
    caller_module_atom = __CALLER__.module
    quote do
      DeeperHub.Logger.__log__(:critical, unquote(message), Keyword.put(unquote(metadata), :module, unquote(caller_module_atom)))
    end
  end
