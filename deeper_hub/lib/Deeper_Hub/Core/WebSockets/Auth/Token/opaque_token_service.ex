    ...
    case :ets.lookup(:opaque_tokens, token) do
      [{^token, token_data}] ->
        ...
        if token_data.purpose != purpose do
          Logger.warn("Tentativa de usar token com finalidade incorreta", %{
            module: __MODULE__,
            expected: purpose,
          ...
          if DateTime.compare(token_data.expires_at, now) == :lt do
            Logger.warn("Tentativa de usar token expirado", %{
              module: __MODULE__,
              purpose: purpose,
      ...
      [] ->
        Logger.warn("Tentativa de verificar token inexistente", %{
          module: __MODULE__,
          purpose: purpose
        })
