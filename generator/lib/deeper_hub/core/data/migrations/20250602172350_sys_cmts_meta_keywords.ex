...
keyword varchar(255) NOT NULL,
  PRIMARY KEY (id)
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de sys_cmts_meta_keywords: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de sys_cmts_meta_keywords: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
