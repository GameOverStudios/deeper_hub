...
cmt_cf int(11) NOT NULL DEFAULT 1,
  PRIMARY KEY (cmt_id)
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_timeline_comments: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_timeline_comments: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
