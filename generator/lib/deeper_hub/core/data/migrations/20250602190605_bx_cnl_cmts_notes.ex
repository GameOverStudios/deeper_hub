...
cmt_pinned int(11) NOT NULL DEFAULT 0,
  PRIMARY KEY (cmt_id)
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_cnl_cmts_notes: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_cnl_cmts_notes: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
