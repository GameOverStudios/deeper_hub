...
street_number varchar(255) NOT NULL,
  PRIMARY KEY (object_id)
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_polls_meta_locations: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_polls_meta_locations: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
