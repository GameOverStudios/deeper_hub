...
expired int(11) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (id)
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_spaces_admins: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_spaces_admins: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
