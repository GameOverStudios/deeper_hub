...
event_id int(11) NOT NULL DEFAULT 0,
  PRIMARY KEY (event_id)
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_timeline_ef_photos: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_timeline_ef_photos: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
