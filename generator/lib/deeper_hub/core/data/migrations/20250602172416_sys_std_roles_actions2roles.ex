...
action_id int(11) unsigned NOT NULL DEFAULT 0,
  ...
  PRIMARY KEY (action_id)
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de sys_std_roles_actions2roles: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de sys_std_roles_actions2roles: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
