...
link_id int(11) NOT NULL DEFAULT 0,
  PRIMARY KEY (id)
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de bx_forum_links2content: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de bx_forum_links2content: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
