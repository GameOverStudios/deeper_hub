...
hash varchar(64) NOT NULL DEFAULT,
  PRIMARY KEY (id)
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela de sys_modules_file_tracks: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
  ...
  def down do
    ...
    case Repo.execute(sql) do
      ...
      {:error, reason} ->
        Logger.error("Falha ao remover tabela de sys_modules_file_tracks: #{reason}", module: __MODULE__)
        {:error, reason}
    end
  end
end
