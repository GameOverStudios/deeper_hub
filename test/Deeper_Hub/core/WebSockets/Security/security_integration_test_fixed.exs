defmodule Deeper_Hub.Core.WebSockets.Security.SecurityIntegrationTest do
  ...
  defp mock_security_supervisor do
    ...
    :meck.expect(SecuritySupervisor, :check_message, fn message, _state, _opts ->
      case message do
  ...
  # Mock do Logger
  defp mock_logger do
    # Verifica se o módulo já está mockado para evitar erro de mock duplicado
    unless :meck.validate(Logger) do
      :meck.new(Logger, [:passthrough, :unstick])
    end
    
    # Mock para funções com um argumento
    :meck.expect(Logger, :error, fn _message -> :ok end)
    :meck.expect(Logger, :warning, fn _message -> :ok end)
    :meck.expect(Logger, :info, fn _message -> :ok end)
    :meck.expect(Logger, :debug, fn _message -> :ok end)
    
    # Mock para funções com dois argumentos
    ...
    :meck.expect(Logger, :debug, fn _message, _opts -> :ok end)

    on_exit(fn -> 
      if :meck.validate(Logger), do: :meck.unload(Logger)
    end)
  end
  ...
  describe "WebSocketHandler.init/2" do
    ...
    test "rejeita conexão quando as verificações de segurança falham" do
      ...
      assert match?({:ok, _req, _state}, result)
  ...
  describe "WebSocketHandler.websocket_handle/2" do
    ...
    test "rejeita mensagem com XSS", %{state: state} do
      ...
      # Deve rejeitar a mensagem - aceita diferentes formatos de resposta
      assert is_tuple(result) and (
        (elem(result, 0) == :reply and is_list(elem(result, 1)) and length(elem(result, 1)) > 0) or
        (elem(result, 0) == :reply and elem(result, 1) == :close)
      )
