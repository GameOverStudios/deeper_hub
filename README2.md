...
O DeeperHub expõe suas funcionalidades através de uma API REST, com mecanismos de proteção e validação.

### 5.1. Limitação de Taxa (Rate Limiting)

O sistema (`Elixir.DeeperHub.API.RateLimit.RateLimiterFacade`, `Elixir.DeeperHub.API.RateLimiter`, `Elixir.DeeperHub.API.RateLimit.Registry`) protege as APIs contra abuso através da limitação de taxa de requisições.

*   **Verificação de Limite:**
    *   Verificar se uma requisição pode ser processada com base no nome da API, ID do cliente e custo da operação (`check/3`).
    *   Retorna `{:ok, remaining}` se permitida, ou `{:error, :rate_limited, retry_after}` se bloqueada.
    *   Suporta chaves de bypass para contornar o rate limiting.
*   **Consumo de Taxa:**
    *   Registrar consumo de taxa sem verificação prévia (para operações permitidas por outros meios) (`consume/3`).
*   **Gerenciamento de Limites:**
    *   Configurar um limitador para uma API específica, definindo o número máximo de requisições, janela de tempo e tempo de bloqueio (`setup/2`).
    *   Obter informações sobre os limites atuais para uma API (`get_limits/1`).
    *   Obter a contagem restante de requisições para um cliente específico e o tempo até o reset (`get_remaining/2`).
    *   Listar todos os clientes atualmente limitados por taxa, opcionalmente para uma API específica (`list_limited_clients/1`).
    *   Resetar o limite para um cliente específico (`reset_client/2`).
*   **Integração com Phoenix/Plug:**
    *   Um Plug (`DeeperHub.API.RateLimiter.call/2`) para integrar o rate limiting diretamente no pipeline do router Phoenix.
    *   Verificação de taxa dentro do contexto de uma conexão Plug (`check_rate/2`), com diferentes tipos de limite (público, autenticado, escrita, sensível) e identificadores de cliente personalizáveis.
*   **Inicialização:** O sistema de rate limiting pode ser inicializado durante o startup da aplicação (`init/0`).

### 5.2. Validação de Requisições da API

O `Elixir.DeeperHub.API.Validation.APIRequestValidator` garante que os dados de entrada das requisições HTTP sejam válidos e sanitizados.

*   **Middleware de Validação:**
    *   Um Plug Phoenix para validar parâmetros de requisição com base em um schema Ecto (`call/2`).
*   **Criação de Schemas de Validação:**
    *   Criar schemas de validação dinamicamente para endpoints da API, definindo campos, tipos, e restrições (obrigatoriedade, tamanho mínimo/máximo, etc.) (`create_schema/1`).
*   **Validação de IDs:**
    *   Validar formatos de IDs (UUID, integer, binary) (`validate_id/2`).
*   **Validação e Sanitização de Requisições:**
    *   Validar e sanitizar parâmetros de requisição HTTP com base em um schema (`validate_request/3`).
    *   Opções para registrar a validação, pular a sanitização e definir domínio para métricas.

### 5.3. Respostas Padronizadas da API

O `Elixir.DeeperHub.Core.ApiResponder` padroniza as respostas da API REST.

*   **Codificação/Decodificação JSON:**
    *   Codificar termos Elixir para JSON e decodificar JSON para termos Elixir, com e sem tratamento de erro (`encode/2`, `encode!/2`, `decode/2`, `decode!/2`).
*   **Formatação de Respostas:**
    *   `format_success/3`: Formata respostas de sucesso, incluindo dados, mensagem opcional e metadados.
    *   `format_error/3`: Formata respostas de erro, incluindo motivo, detalhes e código de erro opcional.
    *   `format_validation_error/2`: Formata respostas de erro de validação, com detalhes dos campos inválidos.
*   **Mapeamento de Erros para Status HTTP:**
    *   Converte motivos de erro internos para códigos de status HTTP padronizados (`error_to_http_status/1`).
...
*   **Funcionalidades não detalhadas na documentação, mas inferidas pelo nome e schemas:**
    *   Definição de Conquistas (`Schema.Achievement`): Gerenciar diferentes tipos de conquistas (nome, descrição, critério, tipo, pontos).
    *   Conquistas de Usuário (`Schema.UserAchievement`): Rastrear quais usuários desbloquearam quais conquistas e quando.
    *   Gerenciar tipos de conquistas (`Lists.Schema.AchievementType`).
*   **Telemetria:** `DeeperHub.Services.Achievements.Telemetry` para monitorar o serviço de conquistas.
