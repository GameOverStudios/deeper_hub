# Migração Elixir: Criar Tabela `bx_persons_votes` (Agregação de Votos para Pessoas)

Este módulo de migração Elixir é responsável por criar a tabela `bx_persons_votes` no banco de dados SQLite. Esta tabela armazena os dados agregados (contagem e soma dos votos) para os perfis de pessoas (`bx_persons_data`).

Esta é um exemplo de uma tabela `TableMain` referenciada em `sys_objects_vote`.

## Código da Migração (`lib/deeper/core/data/migrations/create_bx_persons_votes_table.ex`)

```elixir
defmodule Deeper.Core.Data.Migrations.CreateBxPersonsVotesTable do
  @moduledoc \"\"\"
...
*   `object_id`: Corresponde ao `id` da tabela `bx_persons_data` (o perfil que está sendo avaliado).
*   `count`: Número total de votos recebidos.
*   `sum`: Soma de todos os valores dos votos recebidos.
*   A média (`rate`) é calculada como `sum / count`.