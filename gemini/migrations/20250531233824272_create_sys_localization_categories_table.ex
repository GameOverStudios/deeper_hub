...
defmodule DeeperHub.Core.Data.Migrations.CreateSysLocalizationCategoriesTable do
  ...
  def up do
    ...
    |> tap(fn
      ...
      {:error, reason} ->
        Logger.error("Falha ao criar tabela sys_localization_categories: #{inspect(reason)}",
          module: __MODULE__
        )
    end)
  end
  ...
  def down do
    ...
    |> tap(fn
      {:ok, _} ->
        Logger.info("Tabela sys_localization_categories removida com sucesso.",
          module: __MODULE__
        )

      {:error, reason} ->
        Logger.error("Falha ao remover tabela sys_localization_categories: #{inspect(reason)}",
          module: __MODULE__
        )
    end)
  end
