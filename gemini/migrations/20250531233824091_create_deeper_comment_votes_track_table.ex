...
defmodule DeeperHub.Core.Data.Migrations.CreateDeeperCommentVotesTrackTable do
  ...
  def up do
    ...
    |> tap(fn
      {:ok, _} ->
        Logger.info("Tabela deeper_comment_votes_track criada com sucesso.", module: __MODULE__)

      {:error, reason} ->
        Logger.error("Falha ao criar tabela deeper_comment_votes_track: #{inspect(reason)}",
          module: __MODULE__
        )
    end)
  end
  ...
  def down do
    ...
    |> tap(fn
      {:ok, _} ->
        Logger.info("Tabela deeper_comment_votes_track removida com sucesso.", module: __MODULE__)

      {:error, reason} ->
        Logger.error("Falha ao remover tabela deeper_comment_votes_track: #{inspect(reason)}",
          module: __MODULE__
        )
    end)
  end
