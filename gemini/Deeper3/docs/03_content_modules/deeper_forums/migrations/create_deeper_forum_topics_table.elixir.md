# Migração Elixir: Criar Tabela `deeper_forum_topics`

Este módulo de migração Elixir é responsável por criar a tabela `deeper_forum_topics` no banco de dados SQLite. Esta tabela armazena os tópicos de discussão dentro de cada fórum.

## Código da Migração (`lib/deeper/core/data/migrations/create_deeper_forum_topics_table.ex`)

```elixir
defmodule Deeper.Core.Data.Migrations.CreateDeeperForumTopicsTable do
  @moduledoc \"\"\"
...
*   Depende de `deeper_forums` e `sys_profiles`.
*   `first_post_id` e `last_post_id` referenciam `deeper_forum_posts`, que vem depois. A mesma estratégia de FKs gerenciadas pela aplicação ou adicionadas posteriormente se aplica.
*   `UNIQUE (forum_id, slug)` garante que o slug de um tópico seja único dentro de seu fórum.
*   O índice principal `idx_dft_forum_id_is_sticky_last_post_at` é projetado para a listagem comum de tópicos em um fórum (fixos primeiro, depois por última atividade).