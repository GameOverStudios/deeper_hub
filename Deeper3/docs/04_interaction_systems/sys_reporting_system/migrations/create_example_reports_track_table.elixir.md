# Migração Elixir: (Exemplo) Criar Tabela de Rastreamento de Denúncias (`example_reports_track`)

Este módulo de migração Elixir é um **exemplo** de como uma tabela de rastreamento de denúncias individuais (referenciada por `sys_objects_report.table_track`) seria criada. O nome real da tabela (aqui `example_reports_track`) é dinâmico.

Por exemplo, para o objeto de denúncia `bx_persons_reports`, a `table_track` poderia ser `bx_persons_reports_track`.

**Dependências:** `sys_profiles` (para `author_id` e `checked_by`)

## Código da Migração (`lib/deeper/interaction_systems/reporting/migrations/create_example_reports_track_table.ex`)

```elixir
defmodule Deeper.InteractionSystems.Reporting.Migrations.CreateExampleReportsTrackTable do
  @moduledoc \"\"\"
...
*   `text` está entre aspas.
*   `checked_by` e `status` são importantes para o fluxo de moderação.