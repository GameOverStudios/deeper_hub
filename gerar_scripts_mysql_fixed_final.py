...
# Função para criar migration para uma tabela específica
def criar_migration(tabela, campos, relacoes=None):
    ...
    try:
        with open(arquivo_path, 'w', encoding='utf-8') as arquivo:
            ...
            arquivo.write('      {:error, reason} ->\n')
            # Usando chaves duplas para escapar as chaves na string f-string
            arquivo.write(f'        Logger.error("Falha ao criar tabela de {tabela}: #{{reason}}", module\: __MODULE__)\n')
            arquivo.write('        {:error, reason}\n')
            arquivo.write('    end\n')
            arquivo.write('  end\n\n')
            ...
            arquivo.write('      {:error, reason} ->\n')
            # Usando chaves duplas para escapar as chaves na string f-string
            arquivo.write(f'        Logger.error("Falha ao remover tabela de {tabela}: #{{reason}}", module\: __MODULE__)\n')
            arquivo.write('        {:error, reason}\n')
            arquivo.write('    end\n')
            arquivo.write('  end\n')
            arquivo.write('end\n')
